...
urchin_density_master$Herbivorous_fish <- urchin_guild_master$Herbivorous_fish
urchin_density_master$Urchin_predator <- urchin_guild_master$`Urchin predator`
...
(herb_plot <-
  ggplot() +
   ...
   scale_fill_manual(values = c("Grazing" = "skyblue",
                      "Boring" = "coral") ) +
   labs(title = "g. Herbivore density", fill = "Urchin guild", colour = "Urchin guild", y = "",
        x = bquote('Herbivore density (Fish '*m^-2*')')) +
...
(pred_plot <-
  ggplot() +
   ...
   labs(title = "i. Predator density", fill = "Urchin guild", colour = "Urchin guild", y = "",
        x = bquote('Predator density (Fish '*m^-2*')')) +
...
# ggsave(filename = "output/density_panel_graph_v1.png", full_density_panel, height = 10, width = 34)
```
...
df_ell.cDens.Stratum <- data.frame() #sets up a data frame before running the function.
for(g in levels(cDens.site.scores$Stratum)){
   df_ell.cDens.Stratum <-
     rbind(df_ell.cDens.Stratum, cbind(as.data.frame(with(cDens.site.scores[cDens.site.scores$Stratum==g,],
...
ECMA_data <- data.frame(
  Study = c("bak_ECMA", "conand_ECMA", "csmc_ECMA", "russo_ECMA"),
  Model = c("Power", "Power", "Power", "Power"),  # Specify the model type
  Parameter_a = c(bak_ECMA_param[1, 1], conand_ECMA_param[1, 1], csmc_ECMA_param[1, 1], russo_ECMA_param[1,1]),  # Extract 'a' parameter
  Parameter_b = c(bak_ECMA_param[2, 1], conand_ECMA_param[2, 1], csmc_ECMA_param[2, 1], russo_ECMA_param[2,1]),  # Extract 'b' parameter
  SE_Parameter_a = c(bak_ECMA_param[1, 2], conand_ECMA_param[1, 2], csmc_ECMA_param[1, 2], russo_ECMA_param[1,2]),  # Extract SE for 'a'
  SE_Parameter_b = c(bak_ECMA_param[2, 2], conand_ECMA_param[2,2], csmc_ECMA_param[2,2], russo_ECMA_param[2,2]),  # Extract SE for 'b'
  Location = c("Moorea", "Reunion", "Kenya", "Enewetak")
)
...
csmc_DISA <- read.csv(here::here("erosion", "CSMC_2002_DISA.csv"))
...
urchin_update$Bioerosion <- urchin_erosion$aE*urchin_erosion$biomass.ind^urchin_erosion$bE
...
# SiteNorth Shore  8.964e-06  8.490e-01   0.000    1.000    
# SiteWaikiki     -1.759e+01  4.236e+03  -0.004    0.997  

Anova(erosion_mod)
#                          Chisq Df Pr(>Chisq)    
# Protection.status       1.4578  1    0.22728    
# Site                   68.6388  3  8.350e-15 ***
# Coral                   6.4522  1    0.01108 *  
# Simple_turf             3.9483  1    0.04692 *  
# Macroalgae              0.4076  1    0.52319    
# CCA                     1.2344  1    0.26656    
# Depth_m                 2.1934  1    0.13860    
# Herbivorous_fish        1.3501  1    0.24527    
# Urchin_predator         0.7276  1    0.39367    
# Protection.status:Site 25.0920  3  1.477e-05 ***
...
(turf_plot <-
  ggplot() +
   ...
   geom_text(aes(x = 0, y = 5000, label = "P = 0.046"), hjust = 0, size = 8) +
   labs(title = "b. Turf", y = "",
        x = "Turf cover") +
   my_theme 
 )
...
(coral_plot <-
  ggplot() +
   ...
   geom_text(aes(x = 0, y = 25000, label = "P = 0.011"), hjust = 0, size = 8) +
...
(macroalgae_plot <-
  ggplot() +
   ...
   geom_text(aes(x = 0.01, y = 5000, label = "P = 0.52"), hjust = 0, size = 8) +
...
(CCA_plot <-
  ggplot() +
   ...
   geom_text(aes(x = 0, y = 5000, label = "P = 0.27"), hjust = 0, size = 8) +
...
(depth_plot <-
  ggplot() +
   ...
   geom_text(aes(x = 0, y = 5000, label = "P = 0.13"), hjust = 0, size = 8) +
...
(herb_plot <-
  ggplot() +
   ...
   geom_text(aes(x = 0, y = 5000, label = "P = 0.24"), hjust = 0, size = 8) +
   labs(title = "f. Herbivorous fish", y = "",
        x = bquote('Herbivore density (Fish '*m^-2*')')) +
   my_theme 
 )
...
(pred_plot <-
  ggplot() +
   ...
   geom_text(aes(x = 0, y = 5000, label = "P = 0.39"), hjust = 0, size = 8) +
   labs(title = "f. Urchin predators", y = "",
        x = bquote('Predator density (Fish '*m^-2*')')) +
   my_theme 
 )
